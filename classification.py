...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='efficientnet', 
                        choices=['efficientnet', 'resnet'], help='Model type')
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, device, num_epochs, save_dir):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # if best val F1-score, save model
...
def plot_training_history(history, save_dir):
    ...
    # try to use Arial font, if not exist, use default font
    try:
        arial_font_path = 'Arial.ttf'
        if not os.path.exists(arial_font_path):
            arial_font_path = fm.findfont('Arial')
    ...
    # create figure with single plot combining loss and F1-score
    fig, ax1 = plt.subplots(figsize=(10, 6))
    
    # plot loss on left y-axis
    color_loss_train = 'tab:blue'
    color_loss_val = 'tab:cyan'
    ax1.set_xlabel('Epoch', fontproperties=font_prop, fontsize=12)
    ax1.set_ylabel('Loss', fontproperties=font_prop, fontsize=12, color='tab:blue')
    line1 = ax1.plot(history['train_loss'], label='Training Loss', linewidth=2, 
                     color=color_loss_train, linestyle='-')
    line2 = ax1.plot(history['val_loss'], label='Validation Loss', linewidth=2, 
                     color=color_loss_val, linestyle='-')
    ax1.tick_params(axis='y', labelcolor='tab:blue')
    ax1.grid(True, alpha=0.3)
    
    # create second y-axis for F1-score
    ax2 = ax1.twinx()
    color_f1_train = 'tab:red'
    color_f1_val = 'tab:orange'
    ax2.set_ylabel('F1-Score', fontproperties=font_prop, fontsize=12, color='tab:red')
    line3 = ax2.plot(history['train_f1'], label='Training F1-Score', linewidth=2, 
                     color=color_f1_train, linestyle='--')
    line4 = ax2.plot(history['val_f1'], label='Validation F1-Score', linewidth=2, 
                     color=color_f1_val, linestyle='--')
    ax2.tick_params(axis='y', labelcolor='tab:red')
    
    # combine legends from both axes
    lines = line1 + line2 + line3 + line4
    labels = [l.get_label() for l in lines]
    ax1.legend(lines, labels, loc='center right', prop=font_prop)
    
    plt.title('Model Training History - Loss and F1-Score', 
...
def evaluate_model(model, test_loader, criterion, device, class_names, save_dir):
    ...
    print(f"\nDetailed Classification Metrics:")
    print(f"{'Class':<25} {'Accuracy':<10} {'Precision':<10} {'Recall':<10} {'F1 Score':<10} {'Samples':<10}")
    print("-" * 85)
    for i, class_name in enumerate(class_names):
        accuracy = class_correct[i] / class_total[i] if class_total[i] > 0 else 0.0
        print(f"{class_name:<25} {accuracy:<10.4f} {precision_per_class[i]:<10.4f} {recall_per_class[i]:<10.4f} "
    ...
    # save test results to JSON (without redundant classification_report)
    ...
    with open(os.path.join(save_dir, 'classification_report.txt'), 'w', encoding='utf-8') as f:
        ...
        f.write(f"{'Class':<25} {'Accuracy':<10} {'Precision':<10} {'Recall':<10} {'F1 Score':<10} {'Samples':<10}\n")
        f.write("-" * 85 + "\n")
        for i, class_name in enumerate(class_names):
            accuracy = class_correct[i] / class_total[i] if class_total[i] > 0 else 0.0
            f.write(f"{class_name:<25} {accuracy:<10.4f} {precision_per_class[i]:<10.4f} {recall_per_class[i]:<10.4f} "
                   f"{f1_per_class[i]:<10.4f} {support_per_class[i]:<10}\n")
...
def plot_confusion_matrix(y_true, y_pred, class_names, save_dir):
    ...
    try:
        ...
        arial_italic_font_path = '/usr/share/fonts/truetype/msttcorefonts/Arial_Italic.ttf'
        
        if not os.path.exists(arial_font_path):
            arial_font_path = fm.findfont('Arial')
            arial_italic_font_path = arial_font_path
        
        font_prop = fm.FontProperties(fname=arial_font_path)
        font_prop_italic = fm.FontProperties(fname=arial_italic_font_path)
